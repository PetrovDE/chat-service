...
def test_chart_answer_does_not_claim_success_when_artifact_is_missing() -> None:
    ...
    assert "artifact is not accessible" in message
...
def test_ru_chart_answer_avoids_english_template_fragments() -> None:
    message = build_chart_response_text(
        preferred_lang="ru",
        column_label="created_at",
        chart_rendered=True,
        chart_artifact_available=True,
        chart_fallback_reason="none",
        result_text='[["2026-01", 250], ["2026-02", 420], ["2026-03", 510]]',
        source_scope="orders.xlsx | sheet=Orders | table=orders",
    )

    assert "\u0434\u0430\u043d\u043d\u044b\u0435:" in message.lower()
    assert "\u0442\u0440\u0435\u043d\u0434" in message.lower()
    assert "Top bucket" not in message
    assert "Shape:" not in message
    assert "Used data:" not in message


def test_aggregation_short_circuit_formats_city_sum_without_json_dump() -> None:
    planner = _planner_decision()
    planner_payload = planner.as_dict()
    tabular_sql_result = {
        "status": "ok",
        "prompt_context": "Deterministic tabular SQL result (source of truth): ...",
        "debug": {
            "selected_route": "aggregation",
            "scope_selected_file_name": "spending.xlsx",
            "scope_selected_sheet_name": "Sheet1",
            "scope_selected_table_name": "spending_sheet",
            "tabular_sql": {
                "operation": "sum",
                "metric_column": "amount_rub",
                "group_by_column": "city",
                "result": '[["Kazan", 8427605.94], ["Moscow", 4100000.0], ["SPB", 3150000.5]]',
            },
        },
        "sources": ["spending.xlsx | table=spending_sheet | dataset_v=1 | table_v=1 | sql"],
        "rows_expected_total": 120,
        "rows_retrieved_total": 120,
        "rows_used_map_total": 120,
        "rows_used_reduce_total": 120,
    ...
    _prompt, _rag_used, rag_debug, _docs, _caveats, _sources = build_tabular_success_route_result(
        query="\u043f\u043e\u043a\u0430\u0436\u0438 \u0441\u0443\u043c\u043c\u0443 amount_rub \u043f\u043e city",
        user_id=uuid.uuid4(),
        conversation_id=uuid.uuid4(),
        files=[SimpleNamespace(id="f-city")],
        planner_decision=planner,
        planner_decision_payload=planner_payload,
        expected_chunks_total=0,
        rag_mode="auto",
    ...
    text = str(rag_debug.get("short_circuit_response_text") or "")
    assert rag_debug.get("short_circuit_response") is True
    assert "| city | sum(amount_rub) |" in text
    assert "Kazan" in text
    assert "[[" not in text
    assert "]]" not in text


def test_ru_clarification_message_is_specific_and_clean() -> None:
    message = semantic_clarification.build_retry_clarification(
        reason_code="ambiguous_date_grain_or_source",
        preferred_lang="ru",
    )

    assert "\u043a\u043e\u043b\u043e\u043d\u043a\u0443 \u0434\u0430\u0442\u044b" in message
    assert "metric column and grouping scope" not in message
