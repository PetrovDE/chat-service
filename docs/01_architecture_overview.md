...
- Complex analytics artifact generation limit is now adaptive (base + request-aware boost, bounded by hard cap) to avoid `artifact limit exceeded` on broad analysis requests.
- Executor now enriches incomplete script metrics from dataframe (column profiles/statistics/relationships), so final report stays data-driven even when generated code omits parts of profile payload.
